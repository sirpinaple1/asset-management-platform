...
import type { Category } from '@/api/interface/basedata'
import CategoryModal from './components/CategoryModal.vue'

/** 与路由 name 一致：多页签 keep-alive 缓存键 */
defineOptions({ name: 'basedata-categories' })

const route = useRoute()
const tabsStore = useTabsStore()
const store = useBasedataStore()

const loading = computed(() => store.loading.categories)

/* ---------------- 本地搜索（接口无 keyword，按名称匹配：命中节点及其祖先保留） ---------------- */
const keyword = ref('')

/** 递归过滤：节点名命中保留整个节点；否则仅保留含命中后代的节点 */
const filterTree = (nodes: Category[], kw: string): Category[] => {
...
const treeData = computed(() => {
  const tree = buildTree<Category>(store.categories)
...
const currentRecord = ref<Category>()
...
/* 有子分类/有资产引用时后端 400 拦截，原因由拦截器统一提示 */
...
/* 新增/编辑成功：重拉分类（树形结构、层级关联由后端权威返回） */
const handleSaved = () => {
  void store.fetchCategories()
}

/** 空值统一显示占位符 */
const formatText = (_row: Category, _column: unknown, cellValue: unknown) =>
  ...
  <div class="page">
    <div class="page-container">
      <div class="page-header">
        ...
        <span class="page-subtitle">两级分类结构（四大类 + 子类），资产编码前缀向上继承</span>
      </div>
      ...
      <div class="toolbar">
        ...
        <el-input v-model="keyword" class="search-box" placeholder="搜索分类名称" clearable>
          <template #prefix>
      ...
      >
        ...
        <el-table-column prop="barcodePrefix" label="资产编码前缀" min-width="120" show-overflow-tooltip :formatter="formatText" />
    ...
    <CategoryModal v-model:visible="modalVisible" :data="currentRecord" @success="handleSaved" />
  </div>
</template>
